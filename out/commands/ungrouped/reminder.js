...
exports.command = {
    ...
    execute: function (message, args) {
        return __awaiter(this, void 0, void 0, function () {
            var timeArg, time, usage, content, usage, embed;
            return __generator(this, function (_a) {
                timeArg = args.shift();
                //Get the time to wait
                if (timeArg) {
                    time = ms_1.default(timeArg);
                }
                else {
                    usage = commandUtil_1.GetUsage(this.name);
                    if (usage) {
                        return [2 /*return*/, style_1.QuickEmbed(message, usage)];
                    }
                }
                content = args.join(" ");
                ...
                if (!content) {
                    usage = commandUtil_1.GetUsage(this.name);
                    if (usage) {
                        style_1.QuickEmbed(message, usage);
                    }
                    return [2 /*return*/];
                }
                ...
                setTimeout(function () {
                    if (content) {
                        message.channel.send("Reminder to " + style_1.wrap(content, "`"), { reply: message.author });
                    }
                }, time);
                return [2 /*return*/];
            });
