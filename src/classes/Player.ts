...
export class Player {
  ...
  async startStream(song: ISong) {
    ...
    if (!connection) {
      console.error('Could not connect to the voice channel');
      return;
    }

    connection.play(await ytdl(song.url, { filter: 'audioonly' }), { type: 'opus', highWaterMark: 1 << 32 });
    this.stream = connection.dispatcher;
