...
import { Emoji, GuildMember, Message, MessageEmbed, MessageReaction, PartialUser, Role, User } from 'discord.js'
...
export const command: ICommand = {
  ...
  async execute(message, args) {
    console.log('custom role called...')
    const sections = customRoles.sections
    for (let i = 0; i < sections.length; i++) { setupSection(message, sections, i) }
  },
}

async function setupSection(message, sections, i) {
  ...
  for (let i = 0; i < section.roles.length; i++) {
    await addSection(message, embed, section, i)
  }

  const msg = await message.channel.send(embed)

  for (let r = 0; r < section.roles.length; r++) {
    addSectionRoles(msg, section, r)
  }
}

function addSection(message: Message, embed: MessageEmbed, section: { roles: string | any[] }, roleIndex: number) {
  const role = section.roles[roleIndex]
  ...
  if (roleIndex < section.roles.length - 1) {
    content += '\n\u200b'
  }
...
async function addSectionRoles(msg, section, r) {
...
function addCollector(message: Message, emoji: Emoji, role: Role, sectionRole: Role) {
  ...
  message.guild.client.on('messageReactionRemove', (reaction: MessageReaction, user: User | PartialUser) => {
    const member = message.guild.members.cache.get(user.id)
    if (!member) return console.log('couldnt find member')
    if (reaction.message.id !== message.id || user.bot || reaction.emoji !== emoji) return
    removeRole(member, user)
  })

  function removeRole(member: GuildMember, user: User | PartialUser) {
